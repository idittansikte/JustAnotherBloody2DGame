...
void Collision::update_grid( vector<GameObject*> ObjectList, Point center, bool update_only_visable )
{
  ...
  for( unsigned int i{}; i < ObjectList.size(); ++i)
  {
    ...
    // Check if Object is outside of SCREEN_SIZE size, dont update...
    if (update_only_visable){
      if ( cObject->getRect().x < center.x - SCREEN_WIDTH || cObject->getRect().x > center.x + SCREEN_WIDTH
        || cObject->getRect().y < center.y - SCREEN_HEIGHT || cObject->getRect().y < center.y + SCREEN_HEIGHT ){
          continue;
      }
    }
    ...
    for ( unsigned int cX{cObjectMinX}; cX <= cObjectMaxX; ++cX )
    {
      ...
      for ( unsigned int cY{cObjectMinY}; cY <= cObjectMaxY; ++cY)
      {
        ...
        std::cout << "Added Object to grid: " << "pos_x: "<< cObject->getRect().x << " pos_y: " << cObject->getRect().y << " at grid_pos: " << cX << " " << cY << std::endl;
      }
    }
  }

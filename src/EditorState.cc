...
void EditorState::Init(){
  ...
  std::map<std::string, GameObject*>* creatureList = GameObjectManager::Instance()->GetLoadedList(GameObjectManager::ENEMYLIST);
  std::map<std::string, GameObject*>* platformList = GameObjectManager::Instance()->GetLoadedList(GameObjectManager::PLATFORMLIST);
...
void EditorState::HandleEvents(Game* game){
  ...
  if( Input::getInstance()->is_mouse_down(MOUSE_LEFT) ){  // Handle left mouse key
	...
	break;
      case DELETE:
	found = m_handle_event->GetLevelObjectOnMouse( m_level->GetEditorList() );
	std::cout << found->getUniqueTag() << '\n';
	RemoveObject( found );
	break;
      case MARK:
      case MOVE:
...
GameObject* EditorState::AddObject(GameObject* object){
  ...
  newobject->changeUniqueTag(m_tag++);
...
void EditorState::ChangeLayer(int newLayer){
  ...
  if(m_selected_listobject != nullptr){
    ...
    for( rit= (*m_level->GetEditorList()).rbegin(); rit != (*m_level->GetEditorList()).rend(); ++rit ){ // Find selected object in iterator list
      if(rit->second == m_selected_listobject){ // If found
...
void EditorState::RemoveObject(GameObject* object){
  ...
  printf("searching...\n");
  std::multimap<int, GameObject*>::iterator it_level;
  for( it_level = (*m_level->GetEditorList()).begin(); it_level != (*m_level->GetEditorList()).end(); ++it_level){
...
void EditorState::RemoveLastAdded(){
  ...
  GameObject* tmp;
  std::multimap<int, GameObject*>::reverse_iterator rit;
  for( rit = (*m_level->GetEditorList()).rbegin(); rit != (*m_level->GetEditorList()).rend(); ++rit ){ 
    if( rit->second->getUniqueTag() > highest ){
      highest = rit->second->getUniqueTag();
      tmp = rit->second;
    }
  }
  RemoveObject(tmp);
